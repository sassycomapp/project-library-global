...
The name servers we need to update our domain provider to use can be found by [creating a DNS zone](https://docs.microsoft.com/en-us/azure/dns/dns-getstarted-portal) in our Azure homepage.
...
For more information, Microsoft provides a useful guide to [Azure DNS here](https://docs.microsoft.com/en-us/azure/dns/dns-overview).
...
## [Adding a Network security group](#adding-a-network-security-group)

We open a port to our virtual machine by creating an inbound security rule on a network security group attached to our virtual machine resource. You can find more information on [how to manage ports in Azure here](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/nsg-quickstart-portal).

First, we will create our network security group. This can be done by selecting the `Create a resource` in the [Azure portal](https://docs.microsoft.com/en-us/azure/azure-portal/azure-portal-overview) and searching the marketplace for `Network security group`.
...
Once the network security group is created, we can navigate to the `inbound security rules` and add a new rule. This rule will be to allow inbound traffic over port 443.
